...
- [ ] **Step 4: IP (verificação por IGUALDADE, não por "não-nulo")** — abrir o quiz num celular em rede móvel (não no Wi-Fi da máquina), anotar o IP público real desse celular (ex.: abrir `https://api.ipify.org` no mesmo celular) e comparar com `sessions.client_ip` da sessão criada (SQL Editor: `select client_ip, client_user_agent from sessions order by created_at desc limit 1`). O check "está preenchido?" NÃO basta: o modo de falha perigoso é o rewrite do Vercel trocar o IP pelo do próprio proxy (um IP de datacenter us-east plausível), o que passaria num check de nulo e mandaria pra Meta milhares de conversões com o mesmo IP americano numa campanha mexicana — pior do que não mandar IP. Se `client_ip` for `null` OU diferente do IP do celular: trocar o rewrite do `next.config.js` por um `quiz-app/src/middleware.ts` com matcher `/api/e/:path*` que faça `NextResponse.rewrite(new URL(path, HUB_URL), { request: { headers } })`, onde `headers` é uma cópia dos headers da request com `x-client-ip` = primeiro IP de `x-forwarded-for` (Next 15 não tem mais `request.ip`; usar só os headers). No hub, ler `firstForwardedIp(req.headers.get('x-client-ip') ?? req.headers.get('x-forwarded-for'))`.
